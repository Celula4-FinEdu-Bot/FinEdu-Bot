namespace src.Models;

public sealed class MefRecord
{
    public string? KeyValue { get; set; }

    public string? NivelGobierno { get; set; }
    public string? NivelGobiernoNombre { get; set; }

    public string? Sector { get; set; }
    public string? SectorNombre { get; set; }

    public string? Pliego { get; set; }
    public string? PliegoNombre { get; set; }

    public string? SecEjec { get; set; }

    public string? Ejecutora { get; set; }
    public string? EjecutoraNombre { get; set; }

    public string? DepartamentoEjecutora { get; set; }
    public string? DepartamentoEjecutoraNombre { get; set; }

    public string? ProvinciaEjecutora { get; set; }
    public string? ProvinciaEjecutoraNombre { get; set; }

    public string? DistritoEjecutora { get; set; }
    public string? DistritoEjecutoraNombre { get; set; }

    public string? ProgramaPpto { get; set; }
    public string? ProgramaPptoNombre { get; set; }

    public string? TipoActProy { get; set; }
    public string? TipoActProyNombre { get; set; }

    public string? ProductoProyecto { get; set; }
    public string? ProductoProyectoNombre { get; set; }

    public string? ActividadAccionObra { get; set; }
    public string? ActividadAccionObraNombre { get; set; }

    public string? Funcion { get; set; }
    public string? FuncionNombre { get; set; }

    public string? DivisionFuncional { get; set; }
    public string? DivisionFuncionalNombre { get; set; }

    public string? GrupoFuncional { get; set; }
    public string? GrupoFuncionalNombre { get; set; }

    public string? Meta { get; set; }
    public string? Finalidad { get; set; }
    public string? MetaNombre { get; set; }

    public string? DepartamentoMeta { get; set; }
    public string? DepartamentoMetaNombre { get; set; }

    public string? FuenteFinanciamiento { get; set; }
    public string? FuenteFinanciamientoNombre { get; set; }

    public string? Rubro { get; set; }
    public string? RubroNombre { get; set; }

    public string? CategoriaGasto { get; set; }
    public string? CategoriaGastoNombre { get; set; }

    public string? TipoTransaccion { get; set; }

    public string? Generica { get; set; }
    public string? GenericaNombre { get; set; }

    public string? Subgenerica { get; set; }
    public string? SubgenericaNombre { get; set; }

    public string? SubgenericaDet { get; set; }
    public string? SubgenericaDetNombre { get; set; }

    public string? Especifica { get; set; }
    public string? EspecificaNombre { get; set; }

    public string? EspecificaDet { get; set; }
    public string? EspecificaDetNombre { get; set; }

    public decimal Pia2012 { get; set; }
    public decimal Pim2012 { get; set; }
    public decimal Certificado2012 { get; set; }
    public decimal ComprometidoAnual2012 { get; set; }
    public decimal Comprometido2012 { get; set; }
    public decimal Devengado2012 { get; set; }
    public decimal Girado2012 { get; set; }

    public decimal Pia2013 { get; set; }
    public decimal Pim2013 { get; set; }
    public decimal Certificado2013 { get; set; }
    public decimal ComprometidoAnual2013 { get; set; }
    public decimal Comprometido2013 { get; set; }
    public decimal Devengado2013 { get; set; }
    public decimal Girado2013 { get; set; }

    public decimal Pia2014 { get; set; }
    public decimal Pim2014 { get; set; }
    public decimal Certificado2014 { get; set; }
    public decimal ComprometidoAnual2014 { get; set; }
    public decimal Comprometido2014 { get; set; }
    public decimal Devengado2014 { get; set; }
    public decimal Girado2014 { get; set; }

    public decimal Pia2015 { get; set; }
    public decimal Pim2015 { get; set; }
    public decimal Certificado2015 { get; set; }
    public decimal ComprometidoAnual2015 { get; set; }
    public decimal Comprometido2015 { get; set; }
    public decimal Devengado2015 { get; set; }
    public decimal Girado2015 { get; set; }

    public decimal Pia2016 { get; set; }
    public decimal Pim2016 { get; set; }
    public decimal Certificado2016 { get; set; }
    public decimal ComprometidoAnual2016 { get; set; }
    public decimal Comprometido2016 { get; set; }
    public decimal Devengado2016 { get; set; }
    public decimal Girado2016 { get; set; }
}